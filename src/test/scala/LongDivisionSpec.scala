...
class LongDivisionTester(dut: LongDivision) extends PeekPokeTester(dut) {
  ...
  /*
  * Simple Tests for long Division
  */
  // Test 1
  val val_10_0 = 10.U
  val val_20_0 = 20.U
  ...
  poke(dut.io.numerator, 10.U) //We apply a value to the input
  ...
  expect(dut.io.quotient, 5.U) // 5.0
  ...
  println("---------- Fourth Division ----------")
  poke(dut.io.numerator, 5.U) //We apply a value to the input
  poke(dut.io.denominator, 0.U) //We apply a value to the input
  ...
  println("---------- Finished testing Long Division! ----------")
